...
dummy_list=['tx_difficulty','total_compute','tx_file_type']
def data_selection(drop_columns=[]):
...
def data_preprocessing(change_colDtype={}):
    ...
    for feature in change_colDtype.keys():
        if "bool" in str(data_[feature]):
            data_[feature]= data_[feature].map({True:0,False:1})
        elif 'int' in  str(data_[feature]) or 'float' in str(data[feature]):
